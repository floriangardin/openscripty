...
from maketools.models.tool import ToolORM
from maketools.services.code_executor import CodeExecutorService
from maketools.tools.tool import list_tools
...
5. If you create custom pydantic models for inputs or outputs, you have to define it first in the tool code (before `run` function definition)
6. You don't create a __main__ block in the tool code, it will be wrapped by the backend code
...
Code : ```python
<the python tool code that solve the prompt>
```<end_code>
...
async def create_code_with_ai(
    ctx: RunContextWrapper[MakeToolsContext], input: str
) -> str:
    """
    Create a code for the current tool using AI.
    Uses the tool saved in the database as a spec to generate the code.
    Args:
    ...
    try:
        ...
        current_tool = ctx.context.current_tool
        user_prompt = USER_PROMPT_TEMPLATE.render(
            tool_name=current_tool.name,
            tool_description=current_tool.description,
            current_code=current_tool.code if current_tool.code_generated else None,
            input=input,
        )
        ...
        tool_orm = (
            session.query(ToolORM).filter_by(id=ctx.context.current_tool_id).first()
        )
        if tool_orm:
            tool_orm.code = code
            tool_orm.touched = True
            tool_orm.inputs = input_schema
            tool_orm.outputs = output_schema
            tool_orm.docstring = docstring
            session.commit()
        ...
        # Create tests
        result_message = f"""
        Created code successfully for the tool. 
