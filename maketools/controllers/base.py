...
from maketools.schemas import MakeToolsContext, get_context_from_db
from maketools.services.conversation import ConversationService
from maketools.agents import AgentRegistryInstance
...
async def get_tool(tool_id: str, session: Session) -> Tool:
    """
    Get a tool by id.
    Args:
    ...
    Returns:
        The tool.
    """
    return ToolService.get_tool(session, tool_id)
...
async def update_conversation_with_agent(
    ...
    Returns:
        The conversation.
    """
    context: MakeToolsContext = get_context_from_db(
...
def create_tests(workspace_id: str, tool_id: str, session: Session):
    """
    Create tests for a tool.
    """
    tool = ToolService.get_tool(session, tool_id)
