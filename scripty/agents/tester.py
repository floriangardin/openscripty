...
6. If there is some calls to the `execute_script` function in the function code, you HAVE TO mock the results of all calls to the `execute_script` function. We don't want to test these behaviours, they are already tested. Just create a function that will return the expected output for the `execute_script` function called with these arguments.
...
8. Only when there is calls to èxecute_script`: When patching `execute_script`, always use @patch(f'{__name__}.execute_script', side_effect=mock_execute_script) decorator.
</instructions>
                        
<examples>
<example_1_simple_without_execute_script>
...
```
Assistant response : 
Thoughts: This is a simple function that adds two numbers and writes the result to a file. Since there are no calls to `execute_script`, I don't need to mock anything. I just need to test that the addition is correct.
Code:
```python
import pytest
...
def execute_function(event: dict, output: dict) -> dict:
    ...
    # Use the String Concatenation script to concatenate the first two strings
    result1 = execute_script("String Concatenation", {
        "string_1": string1,
        "string_2": string2
    })

    # Use the String Concatenation script to concatenate the result with the third string
    final_result = execute_script("String Concatenation", {
...
```
Assistant response : 
Thoughts: I need to test the concatenation of the three strings. There are calls to the `execute_script` function, I need to mock them.
...
def mock_execute_script(script_name, inputs):
    # Mock the String Concatenation script behavior
    if script_name == "String Concatenation":
...
@patch(f'{__name__}.execute_script', side_effect=mock_execute_script)
def test_concatenation(mock_script, mock_output):
    inputs = {
        "string1": "Hello ",
        "string2": "World ",
        "string3": "Again"
    ...
    # Verify that execute_script was called twice with the expected arguments
    assert mock_script.call_count == 2
    mock_script.assert_any_call("String Concatenation", {"string_1": "Hello ", "string_2": "World "})
    mock_script.assert_any_call("String Concatenation", {"string_1": "Hello World ", "string_2": "Again"})
...
TEST_CODE_WITH_AI_NAME = "Script test generator"
...
async def test_code_with_ai(
    ...
    """
    Create a test for the current script using AI.
    Returns:
        The test for the script.
    """
    print("Creating code")
    max_iteration = 2
    input = None
    ...
    for _ in range(max_iteration):
        try:
            current_script = ctx.current_script
            user_prompt = USER_PROMPT_TEMPLATE.render(
                ...
                current_code=current_script.code,
                input=input,
            )
            ...
            session = ctx.session
            script_orm = (
                session.query(ScriptORM).filter_by(id=ctx.current_script_id).first()
            )
            if script_orm:
                script_orm.test_code = test_code
                session.commit()
                session.refresh(script_orm)
            
            code_result = await CodeExecutorService.run_test(Script.model_validate(script_orm))
